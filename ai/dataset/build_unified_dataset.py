...
def build_unified(ds14_dir: Path, ds15_dir: Path, output_dir: Path, class_map_path: Path) -> None:
    """DS14 + DS15를 필터링·재인덱싱 후 통합 CSV 저장."""
    ...
    for split in ("train", "val"):
        ...
        ds14_csv = ds14_dir / f"{split}.csv"
        df14 = _load_and_filter(ds14_csv, DS14_TARGET_CLASSES, DS14_REMAP, "DS14")
        if not df14.empty:
            parts.append(df14)
            logger.info(f"[INFO] DS14 {split}: {len(df14)}건")

        ds15_csv = ds15_dir / f"{split}.csv"
        df15 = _load_and_filter(ds15_csv, DS15_TARGET_CLASSES, DS15_REMAP, "DS15")
        if not df15.empty:
            parts.append(df15)
            logger.info(f"[INFO] DS15 {split}: {len(df15)}건")
        ...
        print(f"\n[{split}] 클래스별 분포:")
        for name, cnt in combined["class_name"].value_counts().sort_index().items():
